...
- Exposes a small HTTP API to browse directories and media
- Processes asynchronous scan requests and updates persistent storage
- Supports recurring scan schedules
...
flowchart LR
    ...
    Admin[Admin UI] --> Schedules[Scan schedules]
    Schedules --> Scheduler[Recurring scheduler]
...
- `HTTP API` (`DirectoriesController`, `MediaController`): read and
  manage indexed directories/media.
...
- `Recurring scan scheduler` (`ScanScheduler`): enqueues scan requests
...
- `Persistence` (repositories + adapters): stores directory/media
  hierarchy and query results.
- `Thumbnail adapter` (`ThumbnailsService`): publishes generate/refresh/delete requests.

## High-level flow

1. A directory is registered in the system.
2. A scan request is published to RabbitMQ (manual or recurring).
...
| Browse media                | HTTP requests     | `MediaController`       |
| Manage recurring scans      | Admin requests    | `ScanSchedulesService`  |
| Start scan                  | RabbitMQ message  | `ScanRequestsListener`  |
| Trigger recurring scan      | Scheduler runtime | `ScanScheduler`         |
